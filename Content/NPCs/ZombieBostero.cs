...
using test.Content.Pets.Bloquesito;
...
namespace test.Content.NPCs
{
	...
	public class ZombieBostero : ModNPC
	{
		...
		public override void AI() {
			...
			foreach (Item item in Main.item) {
				...
				if (item.active && !item.beingGrabbed && item.type == ModContent.ItemType<BloquesitoPetItem>() &&	hitbox.Intersects(item.Hitbox)) {
					item.active = false;
		...
		public override void OnKill() {
			...
			while (StolenItems > 0) {
				...
				int droppedAmount = Math.Min(ModContent.GetInstance<BloquesitoPetItem>().Item.maxStack, StolenItems);
				StolenItems -= droppedAmount;
				Item.NewItem(NPC.GetSource_Death(), NPC.Center, ModContent.ItemType<BloquesitoPetItem>(), droppedAmount, true);
			}
		}


		public override bool NeedSaving() {
			return StolenItems >= 1; // Only save if the NPC has more than 10 stolen items, to avoid keeping the NPC in memory if it only has few
		}
